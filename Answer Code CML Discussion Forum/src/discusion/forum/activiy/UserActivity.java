...
public class UserActivity {
	...
	public void seeAllQuestions(UserActivity userActivity, User user) throws NumberFormatException, IOException {
		...
		if (questions.size() == 0 ) { 				// MADE CHANGES FROM if(....)
	...
	//SORT STATEMENT COMMENT ABOVE SAYS ITS SORTING FROM NEWEST TO OLDEST POST BUT THE IMPLEMENTATON OF sort(ArrayList) SORTS ON THE BASIS OF UPVOTES
	...
	/*//NEW SORTING ON THE BASIS OF NEWEST POST TO OLDEST POST
	...
	public void sort(ArrayList<Question> questions) {
		Collections.sort(questions, new Comparator<Question>() {
			public int compare(Question q1, Question q2) {
				...
				return (int) q2.getId() - (int) q1.getId();
			}
		});
	}*/
	...
	public void deleteQuestion(UserActivity userActivity, User user) throws NumberFormatException, IOException {
		...
		User user1 = question.getUser(); //CREATING NEW OBJECT TO STORE user1 = question.getUser(), user1 IS THE USER WHO POSTED THE QUESTION
		...
		if (user.getUserRole() == UserRole.ADMIN) { // the user is an admin
			...
			if(user1.getUserRole() == UserRole.USER || user.getUsername() == user1.getUsername()) { //IF PERSON WHO POSTED THE QUESTION IS A USER OR QUESTION WAS POSTED BY THE MODERATOR ITSELF
			...
			if(user1.getUsername() == user.getUsername()){ // IF PERSON WHO POSTED THE QUESTION IS SAME AS THE PERSON MAKING DELETION REQUEST
				questionService.deleteQuestion(question);
				System.out.println("Question Number " + question.getId() + " Deleted Successfully");
			} else System.out.println("You are not authorised to delete this question");
		}
	...
	private Question getQuestion() throws NumberFormatException, IOException {
		Question question; long inputFromUser = 0;
		while (true) {
			// NumberFormatException HANDLING
			do{try{inputFromUser = Long.parseLong(Utility.inputFromUser());
					break;
				}catch(Exception e){
				System.out.println("Wrong Choice, Only numbers accepted. Try again \n");
				}
			}while(true);
			question = questionService.getQuestionById(inputFromUser);
			if(question != null)
				break;
